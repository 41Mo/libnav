...
void Nav::speed() {
  ns.velocity(0) =
      ns.velocity(0) + (a_enu(0) + (U * sinf32(ns.position(0)) + w_enu(2)) * ns.velocity(1) - co.k1 * (ns.velocity(0) - co.velocity_sns(0))) * dt;
  ns.velocity(1) =
      ns.velocity(1) + (a_enu(1) - (U * sinf32(ns.position(0)) + w_enu(2)) * ns.velocity(0) - co.k1 * (ns.velocity(1) - co.velocity_sns(1))) * dt;
}

void Nav::coordinates() {
  // Latitude
  ns.position(0) = ns.position(0) + (ns.velocity(1) / (R + H) - co.k3 * (ns.position(0) - co.position_sns(0))) * dt;
  // Longitude
  ns.position(1) = ns.position(1) + (ns.velocity(0) / ((R + H) * cosf32(ns.position(0))) - co.k3 * (ns.position(1) - co.position_sns(1))) * dt;
}

void Nav::ang_velocity_body_enu() {
  w_enu(0) = -ns.velocity(1) / (R + H) - (co.k2 / (R + H)) * (ns.velocity(1) - co.velocity_sns(1));
  w_enu(1) = ns.velocity(0) / (R + H) + U * cosf32(ns.position(0)) + (co.k2 / (R + H)) * (ns.velocity(0) - co.velocity_sns(0));
